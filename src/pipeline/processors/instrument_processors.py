...
class InstrumentDataProcessor(ABC):
    ...
    @abstractmethod
    def prepare_data(self, context: ProcessingContext) -> tuple[pd.DataFrame, list[str]]:
        """
    ...
    def _get_relevant_columns(self, df: pd.DataFrame, instrument_variables: list[str]) -> list[str]:
        """
    ...
    def _filter_records_with_data(self, df: pd.DataFrame, relevant_cols: list[str]) -> pd.DataFrame:
        """
        ...
        core_cols = get_core_columns()
        non_core_cols = [
            col for col in relevant_cols if col not in core_cols and not col.endswith("_complete")
        ]
...
class StandardInstrumentDataProcessor(InstrumentDataProcessor):
    ...
    def prepare_data(self, context: ProcessingContext) -> tuple[pd.DataFrame, list[str]]:
        ...
        relevant_cols = self._get_relevant_columns(context.data_df, instrument_variables)
        ...
        instrument_df = self._filter_records_with_data(instrument_df, relevant_cols)
...
class DynamicInstrumentDataProcessor(InstrumentDataProcessor):
    ...
    def prepare_data(self, context: ProcessingContext) -> tuple[pd.DataFrame, list[str]]:
        ...
        Returns:
            Tuple of (filtered DataFrame, list of instrument variables)
        """
        return self._processor.prepare_data(context.data_df, context.primary_key_field)
...
class InstrumentDataCache:
    ...
    def get_processor(self, instrument: str) -> InstrumentDataProcessor | None:
